...
----------------------{ Get Name Bot }-----------------------------
Bot_Name = redis:get(CZAR..":NameBot:")
...
local message = '*💞︙ المنشئيين الاساسين :*\n\n'
...
if #list==0 then  return  "💞*︙* لا يوجد مالك في هذه المجموعه \n√" end
...
EnterCmd = true
local NumberFlood = tonumber(redis:get(CZAR..':Flood_Spam:'..msg.sender_user_id_..':msgs') or 0)
if NumberFlood > 3 then 
redis:setex(CZAR..':Flood_Spam:'..msg.sender_user_id_..':Stoped',10,true)
EnterCmd = false
elseif redis:get(CZAR..':Flood_Spam:'..msg.sender_user_id_..':Stoped') then
EnterCmd = false
else
redis:setex(CZAR..':Flood_Spam:'..msg.sender_user_id_..':msgs',3,NumberFlood+1)
end
return EnterCmd
end
...
redis:sadd(CZAR..'admins:'..msg.chat_id_,data.id_)
end,nil)
end
end
if NumAdmin == 0 then 
...
if redis:get(CZAR..'group:add'..msg.chat_id_) then  return '🎗*︙*المجموعه بالتأكيد ✓️ تم تفعيلها' end
...
redis:setex(CZAR..":ismember:"..msg.sender_user_id_,1500,true)
...
local lock_service = redis:get(CZAR..'lock_service')
...
redis:mset(
CZAR..'group:add'..msg.chat_id_,true,
CZAR..'lock_link'..msg.chat_id_,true, 
CZAR..'lock_id'..msg.chat_id_,true,
CZAR..'lock_spam'..msg.chat_id_,true,
CZAR..'lock_webpage'..msg.chat_id_,true,
CZAR..'lock_markdown'..msg.chat_id_,true,
CZAR..'lock_flood'..msg.chat_id_,true,
...
CZAR..'mute_keyboard'..msg.chat_id_,true,
CZAR..'mute_game'..msg.chat_id_,true,
CZAR..'mute_inline'..msg.chat_id_,true,
CZAR..'lock_username'..msg.chat_id_,true,
CZAR..'num_msg_CZAR'..msg.chat_id_,5, 
CZAR..'replay'..msg.chat_id_,true
)

redis:sadd(CZAR..'group:ids',msg.chat_id_) 
redis:sadd(CZAR..'mtwr_count'..msg.sender_user_id_,msg.chat_id_)

GroupTitle(msg.chat_id_,function(arg,data)
local NameGroup = data.title_
redis:set(CZAR..'group:name'..msg.chat_id_,NameGroup)
...
redis:set(CZAR..'linkGroup'..msg.chat_id_,Gp_Link)
...
if not redis:get(CZAR..'group:add'..msg.chat_id_) then return '√*︙* المجموعه بالتأكيد ✓️ تم تعطيلها' end  
...
if redis:sismember(CZAR..':KARA_BOT:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'💞︙العضو »  '..USERNAME..' \n💞︙الايدي »  `'..UserID..'` \n💞︙انه بالتأكيد منشىء اساسي \n✓️',17,USERCAR) 
end
redis:hset(CZAR..'username:'..UserID,'username',Resolv)
redis:sadd(CZAR..':KARA_BOT:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'💞︙العضو »  '..USERNAME..' \n💞︙الايدي »  `'..UserID..'` \n💞︙تمت ترقيته ليصبح منشىء اساسي في البوت \n✓️',17,USERCAR) 
end

if cmd == "remkara" then
if not redis:sismember(CZAR..':KARA_BOT:'..ChatID,UserID) then
...
if cmd == "unban" then
if not redis:sismember(CZAR..'banned:'..ChatID,UserID) then 
...
if redis:sismember(CZAR..'is_silent_users:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'💞︙العضو »  '..USERNAME..' \n💞︙الايدي »  `'..UserID..'` \n💞︙تم بالتأكيد كتمه  \n✓️',17,USERCAR) 
end
redis:hset(CZAR..'username:'..UserID, 'username', Resolv)
...
if not redis:sismember(CZAR..'is_silent_users:'..ChatID,UserID) then 
...
if redis:sismember(CZAR..'banned:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'💞*︙*العضو »  '..UserName..' \n💞*︙*الايدي »  ``'..UserID..'`` \n💞*︙*تم بالتأكيد حظره \n✓️') 
end
kick_user(UserID, ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
...
if cmd == "dn_sudo" then
if not redis:sismember(CZAR..':SUDO_BOT:',UserID) then return sendMsg(ChatID,MsgID,'💞*︙*العضو » '..USERNAME..' \n💞*︙* الايدي » (``'..UserID..'``)\n💞*︙*انه بالتأكيد ليس مطور \n✓') end
redis:srem(CZAR..':SUDO_BOT:',UserID) 
return SendMention(ChatID,UserID,MsgID,'💞︙العضو »  '..USERNAME..' \n💞︙الايدي »  `'..UserID..'` \n💞︙تم تنزيله من المطورين \n✓️',17,USERCAR) 
end
...
if redis:get(CZAR..'welcome:get'..msg.chat_id_) then welcomex = " مفعل " else welcomex = " معطل " end
if redis:get(CZAR..'lock_id'..msg.chat_id_) then lock_idx = " مفعل " else lock_idx = " معطل " end
if redis:get(CZAR..'replay'..msg.chat_id_)  then replayx  = " مفعل " else replayx  = " معطل " end
if redis:get(CZAR..'lock_woring'..msg.chat_id_) then lock_woringx = " مفعل " else lock_woringx  = " معطل " end
...
.."\n💞︙بالطرد البوتات «» "..(redis:get(CZAR..'lock_bots_by_kick'..msg.chat_id_) or 'false')
.."\n💞︙البوتات «» "..(redis:get(CZAR..'lock_bots'..msg.chat_id_) or 'false')
.."\n💞︙عدد التكرار «» "..(redis:get(CZAR..'num_msg_CZAR'..msg.chat_id_) or 'false')
...
.."\n💞 ︙البصمات «» "..(redis:get(CZAR..'mute_audio'..msg.chat_id_) or 'false')
.."\n💞 ︙الصوت «» "..(redis:get(CZAR..'mute_voice'..msg.chat_id_) or 'false')
